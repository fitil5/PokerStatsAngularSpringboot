...
import com.poker.jwtauthentication.model.IdMovimientoStars;
import com.poker.jwtauthentication.model.MovimientoStars;

@Repository
public interface MovimientoRepositoryStars extends JpaRepository<MovimientoStars,IdMovimientoStars>{
...
@Query(
        value = "select count(distinct(mov.id_mano)) from movimientos_stars mov where mov.ronda=1 and mov.nombre_jugador= ?1 and( mov.tipo='raises' or mov.tipo='calls' or mov.tipo='bets') ", 
        nativeQuery=true
    )
public Double getVpip(String nombre_jugador);
@Query(
        value = "select count(distinct(mov.id_mano)) from movimientos_stars mov where mov.ronda=1 and mov.nombre_jugador= ?1", 
        nativeQuery=true
    )
public Double getVpipTotal(String nombre_jugador);

@Query(
        value = "select (sum(mov.cant)/sum(mov.pot))*100  from movimientos_stars mov where mov.ronda=2 and mov.nombre_jugador= ?1 and  mov.tipo='bets'", 
        nativeQuery=true
    )
public Double getMediaCantFlop(String nombre_jugador);
@Query(
        value = "select (sum(mov.cant)/sum(mov.pot))*100  from movimientos_stars mov where mov.ronda=3 and mov.nombre_jugador= ?1 and mov.tipo='bets'", 
        nativeQuery=true
    )
public Double getMediaCantTurn(String nombre_jugador);
@Query(
        value = "select  (sum(mov.cant)/sum(mov.pot))*100  from movimientos_stars mov where mov.ronda=4 and mov.nombre_jugador= ?1 and mov.tipo='bets'", 
        nativeQuery=true
    )
public Double getMediaCantRiver(String nombre_jugador);
@Query(
        value = "select count(distinct(mov.id_mano))  from movimientos_stars mov where mov.ronda=1 and mov.nombre_jugador= ?1 and( mov.tipo='raises' or mov.tipo='bets')", 
        nativeQuery=true
    )
...
@Query(
        value = "select count(distinct(mov.id_mano)) from movimientos_stars mov where mov.nombre_jugador= ?1 ", 
        nativeQuery=true
    )
